...
function log(msg) {
    fs.appendFileSync('update_v2.log', msg + '\n');
    console.log(msg);
}
...
async function main() {
    try {
        ...
        // Clean credentials in nodes
        const cleanNodes = newDef.nodes.map(n => {
            if (n.type === 'n8n-nodes-base.postgres') {
                // Remove ID but keep structure so n8n knows it needs postgres creds
                if (n.credentials && n.credentials.postgres) {
                    // Set to empty string to forcing manual selection without invalid ID error
        ...
        const payload = {
            ...
            // Not sending settings or active to minimize validation errors
        };
